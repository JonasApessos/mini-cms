...
{
	$usr_id = filter_data($_POST['fin_user_id']);
	$usr_name = filter_data($_POST['fin_user_name']);
	$usr_smoke = filter_data($_POST['fin_user_smoking']);
	$usr_pop = filter_data($_POST['fin_user_people']);
	$usr_comm = filter_data($_POST['fin_user_comm']);
	$date_string = 0;
	$table_list = array();
	$table_list_len = 0;

	$date_string = filter_data($_POST['fin_user_date']);//reservation date
	...
	WHERE ".$prefix."reservation.user_id = ".$usr_id."
	...
	if(!($reservation_row['user_id'] == $_POST['fin_user_id']))//check if in the same date there is already a reservation with the same user and same hour
	{
		...
		(\"".$usr_name."\",".$usr_smoke.",".$usr_pop.",\"".$usr_comm."\",\"".$date_string."\",".$usr_id.");";//insert reservation 
